...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_unique(u'core_thermolog', ['site_id', 'seqn'])

        # Removing unique constraint on 'ThermoSensor', fields ['site', 'seqn']
        db.delete_unique(u'core_thermosensor', ['site_id', 'seqn'])

        # Removing unique constraint on 'AuthenticationToken', fields ['auth_device', 'site', 'token_value']
        db.delete_unique(u'core_authenticationtoken', ['auth_device', 'site_id', 'token_value'])

        # Removing unique constraint on 'AuthenticationToken', fields ['site', 'seqn']
        db.delete_unique(u'core_authenticationtoken', ['site_id', 'seqn'])

        # Removing unique constraint on 'Drink', fields ['site', 'seqn']
        db.delete_unique(u'core_drink', ['site_id', 'seqn'])

        # Removing unique constraint on 'DrinkingSession', fields ['site', 'seqn']
        db.delete_unique(u'core_drinkingsession', ['site_id', 'seqn'])

        # Removing unique constraint on 'Keg', fields ['site', 'seqn']
        db.delete_unique(u'core_keg', ['site_id', 'seqn'])

        # Removing unique constraint on 'ThermoSummaryLog', fields ['site', 'seqn']
        db.delete_unique(u'core_thermosummarylog', ['site_id', 'seqn'])

        # Deleting field 'ThermoSummaryLog.seqn'
        db.delete_column(u'core_thermosummarylog', 'seqn')

        # Deleting field 'SystemEvent.seqn'
        db.delete_column(u'core_systemevent', 'seqn')

        # Deleting field 'Keg.seqn'
        db.delete_column(u'core_keg', 'seqn')

        # Deleting field 'Picture.seqn'
        db.delete_column(u'core_picture', 'seqn')

        # Deleting field 'KegTap.seqn'
        db.delete_column(u'core_kegtap', 'seqn')

        # Deleting field 'DrinkingSession.seqn'
        db.delete_column(u'core_drinkingsession', 'seqn')

        # Deleting field 'Drink.seqn'
        db.delete_column(u'core_drink', 'seqn')

        # Deleting field 'AuthenticationToken.seqn'
        db.delete_column(u'core_authenticationtoken', 'seqn')
        ...
        # Deleting field 'ThermoSensor.seqn'
        db.delete_column(u'core_thermosensor', 'seqn')

        # Deleting field 'Thermolog.seqn'
        db.delete_column(u'core_thermolog', 'seqn')
    ...
    def backwards(self, orm):
        ...
        # User chose to not deal with backwards NULL issues for 'ThermoSummaryLog.seqn'
        raise RuntimeError("Cannot reverse this migration. 'ThermoSummaryLog.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'ThermoSummaryLog', fields ['site', 'seqn']
        db.create_unique(u'core_thermosummarylog', ['site_id', 'seqn'])


        # User chose to not deal with backwards NULL issues for 'SystemEvent.seqn'
        raise RuntimeError("Cannot reverse this migration. 'SystemEvent.seqn' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Keg.seqn'
        raise RuntimeError("Cannot reverse this migration. 'Keg.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'Keg', fields ['site', 'seqn']
        db.create_unique(u'core_keg', ['site_id', 'seqn'])


        # User chose to not deal with backwards NULL issues for 'Picture.seqn'
        raise RuntimeError("Cannot reverse this migration. 'Picture.seqn' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'KegTap.seqn'
        raise RuntimeError("Cannot reverse this migration. 'KegTap.seqn' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'DrinkingSession.seqn'
        raise RuntimeError("Cannot reverse this migration. 'DrinkingSession.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'DrinkingSession', fields ['site', 'seqn']
        db.create_unique(u'core_drinkingsession', ['site_id', 'seqn'])


        # User chose to not deal with backwards NULL issues for 'Drink.seqn'
        raise RuntimeError("Cannot reverse this migration. 'Drink.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'Drink', fields ['site', 'seqn']
        db.create_unique(u'core_drink', ['site_id', 'seqn'])


        # User chose to not deal with backwards NULL issues for 'AuthenticationToken.seqn'
        raise RuntimeError("Cannot reverse this migration. 'AuthenticationToken.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'AuthenticationToken', fields ['site', 'seqn']
        db.create_unique(u'core_authenticationtoken', ['site_id', 'seqn'])

        # Adding unique constraint on 'AuthenticationToken', fields ['auth_device', 'site', 'token_value']
        db.create_unique(u'core_authenticationtoken', ['auth_device', 'site_id', 'token_value'])


        # User chose to not deal with backwards NULL issues for 'ThermoSensor.seqn'
        raise RuntimeError("Cannot reverse this migration. 'ThermoSensor.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'ThermoSensor', fields ['site', 'seqn']
        db.create_unique(u'core_thermosensor', ['site_id', 'seqn'])


        # User chose to not deal with backwards NULL issues for 'Thermolog.seqn'
        raise RuntimeError("Cannot reverse this migration. 'Thermolog.seqn' and its values cannot be restored.")
        # Adding unique constraint on 'Thermolog', fields ['site', 'seqn']
        db.create_unique(u'core_thermolog', ['site_id', 'seqn'])
