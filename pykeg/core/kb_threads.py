...
class FlowProcessingThread(KegbotThread):
  ...
  def run(self):
    while True:
      self._ProcessFlow(self._work_queue.get())
  ...
  def _ProcessFlow(self, flow):
    ...
    d = models.Drink(ticks=flow.Ticks(),
                     volume=volume.Volume(units.RECORD_UNIT),
                     starttime=flow.start, endtime=flow.end, user=flow.user,
                     keg=flow.channel.Keg(), status='valid')
    d.save()
    self._logger.info('Logged drink %i for user "%s": %.2f ounces' % (
      d.id, d.user.username, d.Volume().ConvertTo.ounce))
