...
gflags.DEFINE_string("api_url", _DEFAULT_URL, "Base URL for the Kegweb HTTP api.")

gflags.DEFINE_string("api_key", _DEFAULT_KEY, "Access key for the Kegweb HTTP api.")
...
class Client(object):
    ...
    def _http_request(self, endpoint, params=None, post_data=None):
        ...
        try:
            if post_data:
                r = requests.post(
                    url, params=params, data=post_data, headers=headers, timeout=FLAGS.api_timeout
                )
            else:
                r = requests.get(url, params=params, headers=headers, timeout=FLAGS.api_timeout)
