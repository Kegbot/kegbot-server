...
def site_twitter_redirect(request):
    ...
    return do_redirect(
        request,
        "plugin-twitter-site_twitter_callback",
        "kegadmin-plugin-settings",
        SESSION_KEY_SITE_TWITTER,
    )
...
def site_twitter_callback(request):
    plugin = request.plugins.get("twitter")
    try:
        resource_owner_key, resource_owner_secret = request.session.get(
            SESSION_KEY_SITE_TWITTER, []
        )
    except ValueError:
        ...
        token, token_secret = client.handle_authorization_callback(
            resource_owner_key, resource_owner_secret, request
        )
...
def user_twitter_redirect(request):
    ...
    return do_redirect(
        request,
        "plugin-twitter-user_twitter_callback",
...
def user_twitter_callback(request):
    ...
    try:
        ...
        user_info = client.get_user_info(token, token_secret)
        plugin.save_user_profile(
            request.user, token, token_secret, user_info.screen_name, user_info.id
        )
        messages.success(request, "Successfully linked to @%s" % user_info.screen_name)
