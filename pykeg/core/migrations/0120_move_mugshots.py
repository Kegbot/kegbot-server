...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'User.mugshot'
        db.add_column(u'core_user', 'mugshot',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='user_mugshot', null=True, on_delete=models.SET_NULL, to=orm['core.Picture']),
                      keep_default=False)

        if not db.dry_run:
            for profile in orm['core.UserProfile'].objects.all():
                user = orm['core.User'].objects.get(pk=profile.user_id)
                if profile.mugshot:
                    user.mugshot = profile.mugshot
                    user.save()



    def backwards(self, orm):
        # Deleting field 'User.mugshot'
        db.delete_column(u'core_user', 'mugshot_id')
