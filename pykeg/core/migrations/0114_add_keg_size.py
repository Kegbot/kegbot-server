...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Keg.keg_type'
        db.add_column(u'core_keg', 'keg_type',
                      self.gf('django.db.models.fields.CharField')(default='half-barrel', max_length=32),
                      keep_default=False)

        if not db.dry_run:
            for keg in orm.Keg.objects.all():
                keg.keg_type = keg_sizes.find_closest_keg_size(keg.full_volume_ml)
                keg.save()


    def backwards(self, orm):
        # Deleting field 'Keg.keg_type'
        db.delete_column(u'core_keg', 'keg_type')
