...
class Migration(DataMigration):
    ...
    depends_on = (
        ...
        ('plugin', '0003_to_jsonfield'),
    )

    def forwards(self, orm):
        for old_setting in orm['notification.NotificationSettings'].objects.all():
            print ' ~ migrating notification setting row {}'.format(old_setting.id)
            new_setting = orm['core.NotificationSettings'].objects.create(
                user=old_setting.user,
                backend=old_setting.backend,
                keg_tapped=old_setting.keg_tapped,
                session_started=old_setting.session_started,
                keg_volume_low=old_setting.keg_volume_low,
                keg_ended=old_setting.keg_ended)

        for old_plugin_data in orm['plugin.PluginData'].objects.all():
            print ' ~ migrating plugin data row {}'.format(old_plugin_data.id)
            try:
                # Work around django-jsonfield issue #80: String values are
                # deserialized first, so always hand JSONField a serialized
                # JSON string.
                value = json.dumps(old_plugin_data.value)
            except ValueError:
                print '   malformed value, skipping row: data={} err={}'.format(
                    repr(old_plugin_data.value), e)
            new_data = orm['core.PluginData'].objects.create(
                plugin_name=old_plugin_data.plugin_name,
                key=old_plugin_data.key,
                value=value)
            assert new_data.value == old_plugin_data.value

    def backwards(self, orm):
        orm['notification.NotificationSettings'].objects.all().delete()
        orm['core.PluginData'].objects.all().delete()
    ...
    models = {
        u'core.apikey': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'default': "'f4c6d13949f46c30ab6c8ef069f7364f'", 'unique': 'True', 'max_length': '127'}),
        ...
        u'core.invitation': {
            ...
            'expires_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 5, 30, 0, 0)'}),
            'for_email': ('django.db.models.fields.EmailField', [], {'max_length': '75'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'invite_code': ('django.db.models.fields.CharField', [], {'default': "'e9dee405-4ecc-43f9-b578-fa708ae123a8'", 'unique': 'True', 'max_length': '255'}),
        ...
        u'notification.notificationsettings': {
            ...
            'keg_volume_low': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            'session_started': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'old_notification_settings'", 'to': u"orm['core.User']"})
        },
        u'plugin.plugindata': {
            'Meta': {'unique_together': "(('plugin_name', 'key'),)", 'object_name': 'PluginData'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
    ...
    complete_apps = ['notification', 'plugin', 'core']
    symmetrical = True
