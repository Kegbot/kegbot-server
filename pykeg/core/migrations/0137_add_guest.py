...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            old_guest = orm['core.User'].objects.get(username='guest')
        ...
        for drink in orm['core.Drink'].objects.all().order_by('id'):
            if drink.user:
                continue

            drink.user = guest
            drink.save()
            if drink.time < guest.date_joined:
                guest.date_joined = drink.time
        ...
        for tbl in ('SessionChunk', 'UserSessionChunk', 'UserStats'):
            for row in orm['core.%s' % tbl].objects.filter(user__isnull=True):
                row.user = guest
                row.save()


    def backwards(self, orm):
        guest = orm['core.User'].objects.get(username='guest')

        for drink in orm['core.Drink'].objects.filter(user=guest).order_by('id'):
            drink.user = None
            drink.save()
    ...
    models = {
        ...
        u'core.apikey': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "'33ef9bc4e5cc406f86bc731efb0b89f3'", 'unique': 'True', 'max_length': '127'}),
