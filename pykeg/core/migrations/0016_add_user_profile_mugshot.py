...
class Migration:
    ...
    def forwards(self, orm):
        ...
        db.add_column('core_userprofile', 'mugshot', orm['core.userprofile:mugshot'])
        
        # Set a picture to be the active pic for a user
        for pic in orm.UserPicture.objects.all():
          if pic.active:
            profile = orm.UserProfile.objects.filter(user=pic.user)
            for p in profile:
              p.mugshot = pic
              p.save()
