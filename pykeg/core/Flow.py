...
class Flow:
   ...
   def __init__(self, channel, start = None, user = None,
      ...
      self.user = user
      self._ticks = ticks
      self._volume = units.FluidVolume(ticks, units.UNITS.KbMeterTick)
      self.est_cost = 0.0
   ...
   def SetTicks(self, ticks):
      ...
      self._ticks += diff
      self._volume.IncVolume(diff, units.UNITS.KbMeterTick)
      self._last_ticks = ticks
      if diff != 0:
         self._last_activity = time.time()
      return diff
   ...
   def __init__(self, chanid, valve_relay = None, flow_meter = None,
      ...
      if allow_anon:
         for user in models.User.objects.all():
            ...
            if p and p.HasLabel('__default_user__'):
               self.anon_user = user
         if self.anon_user is not None:
            self.logger.info('Anonymous user: %s' % self.anon_user)
         else:
