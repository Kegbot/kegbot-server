...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for token in orm.AuthenticationToken.objects.filter(auth_device='core.onewire'):
            token.token_value = token.token_value.lower()
            token.save()
        for token in orm.AuthenticationToken.objects.filter(auth_device='core.rfid'):
            token.token_value = token.token_value.lower()
            token.save()
    ...
    models = {
        ...
        'core.userprofile': {
            ...
            'api_secret': ('django.db.models.fields.CharField', [], {'default': "'ba379c59f0db1fb3ef9b6548fb31fff1'", 'max_length': '256', 'null': 'True', 'blank': 'True'}),
