...
class ChangeKegForm(forms.Form):
    keg_size = forms.ChoiceField(
        choices=keg_sizes.CHOICES, initial=keg_sizes.HALF_BARREL, required=True
    )

    initial_volume = forms.FloatField(
        label="Initial Volume", initial=0.0, required=False, help_text="Keg's Initial Volume"
    )
    ...
    style_name = forms.CharField(
        required=True, label="Style", help_text="Example: Pale Ale, Stout, etc."
    )
...
class BeverageForm(forms.ModelForm):
    ...
    new_image = forms.ImageField(required=False, help_text="Set/replace image for this beer type.")
