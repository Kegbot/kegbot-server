...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'FlowMeter.ticks_per_ml'
        db.add_column(u'core_flowmeter', 'ticks_per_ml',
                      self.gf('django.db.models.fields.FloatField')(default=5.4),
                      keep_default=False)

        if not db.dry_run:
            # Migrate calibrations from tap to meter.
            for meter in orm['core.FlowMeter'].objects.all():
                tap = meter.tap
                if tap:
                    meter.ticks_per_ml = 1.0/tap.ml_per_tick
                    meter.save()


    def backwards(self, orm):
        # Deleting field 'FlowMeter.ticks_per_ml'
        db.delete_column(u'core_flowmeter', 'ticks_per_ml')
    ...
    models = {
        ...
        u'core.apikey': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "'4f1b7436d1c22beaf77502e104ee5207'", 'unique': 'True', 'max_length': '127'}),
