...
class Migration:
    ...
    def forwards(self, orm):
        ...
        db.create_table('core_thermosensor', (
            ('id', orm['core.thermosensor:id']),
            ('raw_name', orm['core.thermosensor:raw_name']),
            ('nice_name', orm['core.thermosensor:nice_name']),
        ))
        db.send_create_signal('core', ['ThermoSensor'])
        
        # Adding field 'Thermolog.sensor'
        db.add_column('core_thermolog', 'sensor', orm['core.thermolog:sensor'])
        
        for entry in orm.Thermolog.objects.all():
          sensor, created = orm.ThermoSensor.objects.get_or_create(
              raw_name=entry.name,
              defaults={'nice_name':entry.name})
          entry.sensor = sensor
          entry.save()
    ...
    def backwards(self, orm):
        ...
        db.delete_table('core_thermosensor')
        
        # Deleting field 'Thermolog.sensor'
        db.delete_column('core_thermolog', 'sensor_id')
    ...
    models = {
        ...
        'core.thermolog': {
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '128'}),
            'sensor': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['core.ThermoSensor']", 'null': 'True'}),
